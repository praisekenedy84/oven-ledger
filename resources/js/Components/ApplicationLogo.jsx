export default function ApplicationLogo({
    ...
    return (
        <div className={`flex items-center gap-2 ${className}`}>
            {src ? (
                <img src={src} alt="" className="h-9 w-9 shrink-0 rounded-md object-contain bg-white/10 p-0.5" />
            ) : (
            ...
            {showText && (
                <span className="font-brand truncate text-[1.15rem] font-semibold leading-none tracking-tight">
                    {name}
                </span>
            )}
        </div>
